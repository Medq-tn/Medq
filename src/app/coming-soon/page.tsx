...
export default function ComingSoonPage() {
  ...
  {/* background accents - soft floating blobs in brand colors */}
  ...
  <div className="pointer-events-none absolute -bottom-40 -right-40 w-[36rem] h-[36rem] rounded-full bg-medblue-300/50 blur-3xl animate-[float_15s_ease-in-out_infinite_reverse]" />
      ...
      <main className="relative z-10">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-12 md:py-20">
          ...
          <div className="mx-auto w-28 h-28 md:w-32 md:h-32 rounded-full bg-white/80 backdrop-blur-sm border border-slate-200 flex items-center justify-center relative shadow-sm">
            ...
            <PartyPopper className={`absolute -top-3 -right-3 w-6 h-6 text-amber-500 transition-all ${burst ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-1'}`} />
          </div>

          <h1 className="mt-8 text-center text-4xl md:text-5xl font-extrabold tracking-tight text-slate-900">
            Félicitations.
          </h1>
          <p className="mt-3 text-center text-slate-600 text-lg max-w-2xl mx-auto">
            Votre compte a été créé avec succès et vous faites maintenant partie de notre communauté.
            Vous serez parmi les premiers à découvrir notre plateforme.
          </p>
          {/* Removed email notification line */}
      ...
      <style jsx global>{`
        @keyframes confettiDrop {
          0% { transform: translate3d(0,-20%,0) rotate(0deg) scale(var(--s,1)); opacity: 0; }
          10% { opacity: 1; }
          100% { transform: translate3d(0,120vh,0) rotate(360deg) scale(var(--s,1)); opacity: 0; }
        }
        ...
        .logo-shine { animation: logoShine 2.2s ease-in-out 1.2s infinite; }
      `}</style>
    </div>
  );
}
