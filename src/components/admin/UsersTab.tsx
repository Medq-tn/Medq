...
export function UsersTab() {
  ...
  const fetchUsers = async (page = 1, searchTerm = search, role = roleFilter) => {
    setIsLoading(true);
    try {
      const result = await getUsers(page, 20, searchTerm);
    ...
    <div className="space-y-6 sm:space-y-8 animate-fade-in">
                  ...
                  <div key={user.id} className="p-4 space-y-3">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Badge className={`${user.hasActiveSubscription ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'} border-none font-medium rounded-full px-2.5 py-0.5 text-[10px]`}>
                    ...
                    <div className="flex items-center justify-between text-[11px] text-muted-foreground">
                      <div className="flex items-center gap-1">
                        ...
                        <span>{user.profileCompleted ? 'Completed' : 'Pending'}</span>
                      </div>
                      <div className="flex items-center gap-1">
                ...
                <Table className="text-xs sm:text-sm">
                  <TableHeader className="bg-muted/40">
                    <TableRow>
                      ...
                      <TableHead className="font-semibold uppercase tracking-wide text-[10px] sm:text-xs text-muted-foreground">Subscription</TableHead>
                      <TableHead className="font-semibold uppercase tracking-wide text-[10px] sm:text-xs text-muted-foreground">Profile</TableHead>
                  ...
                  <TableBody>
                    ...
                    <TableRow key={user.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <TableCell>
                        <Badge className={`${user.hasActiveSubscription ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'} border-none font-medium rounded-full px-2.5 py-0.5 text-[10px] sm:text-xs`}>
                          {user.hasActiveSubscription ? 'Active' : 'Inactive'}
                        </Badge>
                      </TableCell>
                      <TableCell>
      ...
      {pagination && (
        <div className="text-center text-[11px] sm:text-xs text-muted-foreground">
          Showing {((currentPage - 1) * 20) + 1} to {Math.min(currentPage * 20, pagination.totalItems)} of {pagination.totalItems} results
        </div>
      )}
    </div>
  );
