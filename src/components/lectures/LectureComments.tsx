...
export function LectureComments({ lectureId }: LectureCommentsProps) {
  ...
  const handleSubmitComment = async () => {
    if (!user?.id || !newComment.trim()) {
      toast({
        title: "Erreur",
        description: "Veuillez vous connecter et saisir un commentaire.",
        variant: "destructive",
      });
      return;
    }
  ...
  toast({ title: 'Réponse ajoutée', description: 'Votre réponse a été publiée.' });
        setReplyTo(null);
        setReplyAnon(false);
      } else {
        const errorData = await response.json();
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            {isOwner && editingComment !== comment.id && (
              <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => startEditing(comment)} title="Modifier"><Edit2 className="h-3.5 w-3.5" /></Button>
            )}
            {(isOwner || isAdmin) && (
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" title="Supprimer" onClick={() => setDeleteTarget(comment)}><Trash2 className="h-3.5 w-3.5" /></Button>
                </AlertDialogTrigger>
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>Supprimer le commentaire</AlertDialogTitle>
                    <AlertDialogDescription>
                      Cette action supprimera définitivement le commentaire{comment.replies && comment.replies.length > 0 ? ' et ses réponses' : ''}. Cette action est irréversible.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
      ...
      <CardContent className="space-y-5 p-4 pt-0">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center text-center py-10 rounded-xl border bg-background/40">
            ...
            <h3 className="font-semibold mb-1">Aucun commentaire pour le moment</h3>
            <p className="text-sm text-muted-foreground max-w-sm">Soyez le premier à partager votre avis ou poser une question sur ce cours.</p>
          </div>
        ) : (
          <div className="space-y-3">
...
function InlineReplyEditor({ parent, onSubmit, onCancel, defaultAnon }: InlineReplyEditorProps) {
  ...
  return (
    <div className="mt-3 space-y-2">
      <div className="flex items-center justify-between">
  <span className="text-xs text-muted-foreground">En réponse à {parent.isAnonymous ? 'Anonyme' : parent.user?.name || 'Utilisateur'}</span>
  ...
  placeholder="Rédiger une réponse…"
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={() => { if (value.trim()) onSubmit(value, anon); }} disabled={!value.trim()}>Répondre</Button>
        </div>
      </div>
    </div>
  );
