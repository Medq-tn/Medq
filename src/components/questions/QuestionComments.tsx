    ...
    return (
      <div className="mt-3 space-y-2">
        ...
        <div className="flex gap-2 justify-end">
          <Button size="sm" variant="outline" onClick={cancelReply}>Annuler</Button>
          <Button size="sm" disabled={disabled} onClick={() => add(parentId, value)}>Publier la réponse</Button>
        </div>
      </div>
    );
  };
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 text-xs text-muted-foreground flex-wrap">
              ...
              {c.updatedAt && c.updatedAt !== c.createdAt && (<span className="italic">(modifié)</span>)}
            </div>
            {!isEditing ? (
              ...
              <div className="mt-2">
                ...
                <div className="mt-2 flex gap-2 justify-end">
                  ...
                  <Button size="sm" disabled={!editText.trim()} onClick={() => saveEdit(c.id)}>Enregistrer</Button>
                </div>
              </div>
            )}
            <div className="mt-2 flex gap-2">
              <Button variant="ghost" size="sm" className="h-7 px-2 text-xs" onClick={() => startReply(c.id)}>Répondre</Button>
              {isOwner && !isEditing && (
                <Button variant="ghost" size="sm" className="h-7 px-2 text-xs" title="Modifier" onClick={() => beginEdit(c)}>Modifier</Button>
              )}
              {canDelete && !isEditing && (
                <Button variant="ghost" size="sm" className="h-7 px-2 text-xs text-destructive" title="Supprimer" onClick={() => remove(c.id)}>Supprimer</Button>
              )}
            </div>
      ...
      <div className="rounded-xl border bg-muted/30">
        ...
        <div className="p-3 sm:p-4 space-y-4">
            ...
            <div className="rounded-lg border bg-background/70 p-2 sm:p-3">
              ...
              <div className="mt-2 flex items-center justify-between text-xs text-muted-foreground">
                ...
                <Button size="sm" disabled={!canPostRoot} onClick={() => add()}>
                  {submitting ? (<><Loader2 className="h-3.5 w-3.5 mr-1 animate-spin" /> Publication…</>) : (<><Send className="h-3.5 w-3.5 mr-1" /> Publier</>)}
                </Button>
              </div>
            </div>
          ) : (
          ...
          {!loading && comments.length === 0 && (
            <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">Aucun commentaire pour le moment.</div>
          )}
