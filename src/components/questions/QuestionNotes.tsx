...
export function QuestionNotes({ questionId }: QuestionNotesProps) {
  ...
  const save = async (silent = false) => {
    if (!user?.id) {
      if (!silent) toast({ title: 'Connexion requise', description: 'Veuillez vous connecter pour enregistrer des notes', variant: 'destructive' });
      return;
    }
    try {
      setSaveState('saving');
      ...
      setLastSavedAt(new Date());
      setSaveState('saved');
  if (!silent) toast({ title: 'Enregistré', description: 'Votre note a été enregistrée.' });
      // revert to idle after a moment
      setTimeout(() => setSaveState('idle'), 1200);
    } catch {
      setSaveState('error');
  if (!silent) toast({ title: 'Erreur', description: 'Échec de l’enregistrement de la note', variant: 'destructive' });
    }
  };
  ...
  const clearNote = async () => {
    ...
    await save(true);
  toast({ title: 'Effacé', description: 'Votre note a été supprimée.' });
  };

  const status = (
    <div className="flex items-center gap-1 text-xs text-muted-foreground">
      {saveState === 'saving' && (<><Loader2 className="h-3.5 w-3.5 animate-spin" /> Enregistrement…</>)}
      {saveState === 'saved' && (<><CheckCircle2 className="h-3.5 w-3.5 text-green-600" /> Enregistré</>)}
      {saveState === 'error' && (<span className="text-red-600">Échec de l’enregistrement</span>)}
      {saveState === 'idle' && lastSavedAt && !hasChanges && (
        <span>Enregistré à {lastSavedAt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
      )}
    </div>
  );
