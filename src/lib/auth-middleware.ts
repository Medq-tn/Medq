  ...
  return async (request: NextRequest, ...args: T) => {
    ...
    if (!authenticatedRequest) {
      if ((request as any)._dbUnavailable) {
        return NextResponse.json({ error: 'Service indisponible : base de données injoignable' }, { status: 503 });
      }
...
export function requireAdmin<T extends any[]>(
  ...
  return async (request: NextRequest, ...args: T) => {
    ...
    if (authenticatedRequest.user?.role !== 'admin') {
      return NextResponse.json({ error: 'Interdit — accès administrateur requis' }, { status: 403 });
    }
    return handler(authenticatedRequest, ...args);
  };
}
...
export function requireMaintainerOrAdmin<T extends any[]>(
  ...
  return async (request: NextRequest, ...args: T) => {
    ...
    if (role !== 'admin' && role !== 'maintainer') {
      return NextResponse.json({ error: 'Interdit — rôle maintainer ou administrateur requis' }, { status: 403 });
    }
    return handler(authenticatedRequest, ...args);
  };
}
...
export async function verifyAuth(request: NextRequest): Promise<{ success: boolean; userId?: string; error?: string }> {
  try {
    ...
    if (!token) {
      return { success: false, error: 'Aucun jeton fourni' };
    }
